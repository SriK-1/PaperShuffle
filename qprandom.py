...
def get_set_name(index):
    """Converts numbers to Excel-style column names (0->A, 25->Z)."""
    result = ""
    while index >= 0:
...
class QuestionGeneratorApp(ctk.CTk):
    def __init__(self):
        ...
        self.minsize(850, 700) # Prevents user from squishing the UI too small
        self.resizable(True, True) # Allows maximizing and resizing!
        ...
        self.tabview.pack(padx=20, pady=5, fill="both", expand=True) # expand=True makes it stretch
        ...
        self.file_listbox.pack(padx=10, pady=5, fill="both", expand=True) # Responsive

        # Tab 2: Direct Text / Preview
        ...
        self.textbox.pack(padx=10, pady=5, fill="both", expand=True) # Responsive
    ...
    def browse_files(self):
        ...
        if new_files:
            ...
            self.filepaths = list(dict.fromkeys(self.filepaths))
    ...
    def write_question_to_doc(self, doc, index, q_text, include_answers):
        ...
        for line in lines[1:]:
            ...
            if in_ans:
                if not include_answers: continue 
